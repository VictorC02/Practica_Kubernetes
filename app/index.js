...
app.use('/static', express.static('/usr/share/app/static'));
...
app.get('/health', async (req, res) => {
  try {
    ...
    res.status(200).json({ status: 'ok', message: 'La App y la BD están sanas' });
  } catch (error) {
    res.status(500).json({ status: 'error', message: 'Conexión con la BD fallida' });
  }
});
